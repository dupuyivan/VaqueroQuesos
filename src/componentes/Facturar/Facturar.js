...
const Facturar = ({ idPermiso }) => {
  ...
  const handleGuardar = ({ A, B, index }) => {
    ...
    pedidosTemp[index].disabled = false;
  ...
  const handleGuardarPedido = (index) => async (e) => {
    ...
    try {
      ...
      let copiaDePedidosAFacturar = [...pedidosAFacturarFiltrados];
      copiaDePedidosAFacturar[index].disabled = true;
      setPedidosAFacturarFiltrados(copiaDePedidosAFacturar);
